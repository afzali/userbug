...
رابط روی `http://127.0.0.1:4174` بالا می‌آید و مرورگر را خودش باز می‌کند.
صفحهٔ اول فهرست پروژه‌هاست؛ با ورود به یک پروژه، فضای کاریِ آن باز می‌شود
(`/projects/<کلید>` و زیرمسیرهای `triage`, `compare`, `files`). از همان‌جا
می‌توانید اجرا را شروع کنید، قدم و عکس و خطا را زنده ببینید، سناریو را با متن
ساده و هوش مصنوعی بسازید، فایل سناریو/کانفیگ را ویرایش کنید، یافته‌ها را تریاژ
کنید و دو run را مقایسه کنید.

داده‌ها همچنان فایل‌اند: `runs/` برای اجراها، `triage/` برای تصمیم‌های تریاژ و
`schedules/` برای زمان‌بندی‌ها.

### زمان‌بندی

کارت «زمان‌بندی» در فضای کاری پروژه، یا از خط فرمان:

```bash
node bin/userbug.js schedule add nightly --target nepi --time 02:00
node bin/userbug.js schedule list
node bin/userbug.js schedule run nightly    # بدون منتظر ماندن تا دو بامداد
node bin/userbug.js schedule remove nightly
```

زمان‌بندِ واقعی Task Scheduler ویندوز است. اگر فهرست گفت «در زمان‌بند نیست»،
یعنی فایلِ تعریف هست ولی تسک نیست — مثلاً از ماشین دیگری آمده یا دستی پاک شده؛
دوباره `add` کنید.
...
**`npm run ui:build` نگیرید وقتی رابط بالاست.** بیلد فایل‌های
`ui/build/server/chunks/*` را با نامِ هش‌دار تازه جایگزین می‌کند، ولی سرورِ در
حال اجرا manifest قدیمی را در حافظه دارد. نتیجه‌اش `500` روی اندپوینت‌ها با
`ERR_MODULE_NOT_FOUND` است — که شبیه باگِ کد به نظر می‌رسد و نیست. سرور را
ببندید، بعد بیلد. `node scripts/up.mjs` خودش همین ترتیب را دارد.

**تغییر در `src/` را روی بیلد رابط بسنجید، نه فقط با CLI.** رابط ماژول‌های
`src/` را باندل می‌کند، پس هر چیزی که به محلِ فایل تکیه کند (مثل
`import.meta.dirname`) در CLI درست و در رابط غلط کار می‌کند. یک بار همین
`.env` را از دید رابط ناپدید کرد.

**سنجش آفلاین روی سرور توسعه معنا ندارد.** ویت ماژول‌ها را در لحظه از شبکه
می‌دهد و `+layout.svelte` سرویس‌ورکر را فقط در حالت production ثبت می‌کند — در
dev حتی سرویس‌ورکرهای جامانده را هم برمی‌دارد. پس روی هدف `nepi` این سنجش
همیشه یافتهٔ قلابی می‌داد.

برای همین هدف `nepi-preview` هست. در پوشهٔ نپی:
...
بعد `node bin/userbug.js run nepi-preview`.

`build:noversion` عمدی است: `build` معمولی نسخه را از تعداد کامیت‌ها جلو
می‌برد و فایل changelog می‌سازد؛ یک بیلدِ آزمایشی نباید تاریخِ نسخه‌های پروژه
را دست بزند.

**روی بیلد تولیدی `query` کار نمی‌کند.** داورِ وضعیتِ هدف `nepi` ماژول
دیتابیس را با مسیر سورس import می‌کند و آن مسیر فقط در dev هست. `nepi-preview`
عمداً `state.sql` ندارد تا ادعای کارکردنش را نکند.
