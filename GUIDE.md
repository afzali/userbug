...
## ۰. پروژهٔ تازه؟ از اینجا شروع کنید

سه فرمان، به همین ترتیب. هرکدام خودش مفید است و منتظر بعدی نمی‌ماند.

```bash
userbug init <کلید> --base-url <آدرس> --source <پوشهٔ سورس>
```

```bash
userbug learn <کلید> --dry
```

`--dry` رایگان و بی‌مدل است: فهرست روت‌ها و استک را از سورس درمی‌آورد و
**چیزی ذخیره نمی‌کند**. اگر اینجا صفر روت دید، فراخوانیِ مدل فقط پول سوزاندن
است. بدون `--dry` یک بار مدل را صدا می‌زند و معنا و پرسش‌ها را هم می‌گیرد.

```bash
userbug tour <کلید>
```

مرورگر باز می‌شود. **شما** لاگین می‌کنید و منوها را می‌گردید؛ ابزار تماشا
می‌کند و در پایان چهار چیز می‌نویسد: صفحه‌ها با جملهٔ خودتان، یک پیش‌نویس
سناریو، کشِ آموخته (که دیگر مدل نمی‌خواهد)، و پروندهٔ شناخت.

بعدش:

```bash
userbug knowledge <کلید> --questions
userbug knowledge <کلید> --answer 1 --as "جوابتان"
```

پرسش‌هایی که مدل از سورس نفهمید. جوابِ شما `by: user` می‌گیرد و از آن به بعد
هیچ حدسی عوضش نمی‌کند. **پرسیدن ارزان‌تر از حدس زدن است.**

> همهٔ این‌ها از رابط هم هست: `/projects/<کلید>/knowledge` و
> `/projects/<کلید>/tour`. طرح و دلیلِ هر تصمیم در
> [ONBOARDING.md](ONBOARDING.md).

### فرمان‌های دیگرِ شناخت

| کار | فرمان |
|---|---|
| چکِ همگانی: حالت و سروصدا | `userbug checks <کلید>` |
| ناوردا (باگِ منطقی از schema) | `userbug invariants <کلید>` |
| حساب ذخیره‌شده (رمز از متغیر محیطی) | `userbug accounts <کلید>` |
| مستندِ بیرونی | `userbug docs <کلید> --add <آدرس>` |
| تاریخچهٔ شناخت | `userbug knowledge <کلید> --history` |
...
ببندید، بعد بیلد. `start.bat` (یعنی `node scripts/serve.mjs`) خودش همین ترتیب را دارد.
