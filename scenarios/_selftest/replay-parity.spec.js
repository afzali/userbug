/**
 * خودآزمای بازپخشِ خزش.
 *
 * ── این فایل روزی دربارهٔ **دو** مفسر بود ──
 *
 * `src/scenario/run.js` سناریو را اجرا می‌کرد و `src/map/replay.js` همان
 * سناریو را برای خزش بازپخش می‌کرد. مقدمهٔ `replay.js` از روز اول هشدار
 * داده بود: «دو مسیرِ اجرا که دیر یا زود واگرا می‌شوند».
 *
 * آن هشدار با برداشتنِ اجرای YAML موضوعیت خود را از دست داد — ولی نه
 * کاملاً: `replay.js` ماند و حالا **تنها مفسرِ باقی‌مانده** است. خزش برای
 * کاوش باید مرورگر را برانَد، و آن اجرای تست نیست. پس بندهای زیر که
 * دربارهٔ خودِ بازپخش‌اند، همچنان معنا دارند؛ آن‌هایی که سمتِ دیگر را
 * می‌سنجیدند، رفتند.
 *
...
test('`then` هم کنارِ `when` کار می‌کند هم داخلش', () => {
  /**
   * تورفتگیِ YAML این را آسان اشتباه می‌کند. پیش‌تر شکلِ تودرتو بی‌صدا هیچ
   * کاری نمی‌کرد: شرط درست ارزیابی می‌شد، `then` خالی بود، و کلِ بلوک رد
   * می‌شد بی هیچ خطایی.
   *
   * سناریوی ورودِ نپی دقیقاً همین را داشت: سبز بود و هرگز وارد نمی‌شد.
   *
   * سمتِ دومِ این ادعا — همین قاعده در `src/scenario/run.js` — با برداشتنِ
   * اجرای YAML رفت. بازپخش تنها جایی است که این شکل هنوز خوانده می‌شود.
   */
