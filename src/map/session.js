...
export class MapSession extends EventEmitter {
  ...
  async start() {
    ...
    await this.store.init({
      ...
      kind: 'map',
      /**
       * ── چرا خزش هم بنچ می‌گیرد ──
       *
       * «دورِ بررسی» یعنی چند کار زیرِ یک اسم، و کاری که آدم واقعاً
       * می‌خواهد معمولاً ترکیبی است: سناریوها را بگیر **و** دوباره بخز
       * ببین چیزی تازه پیدا می‌شود. تا امروز `--bench` فقط به اجرای
       * سناریو می‌چسبید، پس نیمی از هر دور بی‌نام می‌ماند و در فهرست از
       * خزشِ هفتهٔ پیش جدا نمی‌شد.
       */
