...
import { CLOSERS } from '../observe/blockers.js';
...
export function buildEntry({ steps = [], accountId = '', name = 'ورود' } = {}) {
  ...
  for (; index < steps.length && form.length < 10; index++) {
    ...
    /**
     * بستنِ پنجره **دور ریخته می‌شود**، نگه داشته نمی‌شود.
     *
     * ── چرا این قاعده برعکس شد ──
     *
     * اول هر کلیکِ میانِ فرم را نگه می‌داشتیم. نتیجه‌اش روی نپی این شد:
     * ضبطِ گشت `click «نشان نده»` و `click «بستن»` داشت — دو مودالی که
     * فقط بارِ اول می‌آیند — و همان‌ها بی‌شرط داخلِ مسیرِ ورود نشستند.
     * بارِ دوم که خزش به خانه برگشت، آن دکمه‌ها نبودند و
     * `locator.click: Timeout` کلِ خزش را کشت. سه اجرا پشتِ سرِ هم با
     * **صفر قدم** تمام شد.
     *
     * و نگه داشتنشان اصلاً لازم نبود: `dismissBlockers` همان کار را
     * **مشروط** می‌کند — اگر پنجره‌ای بود می‌بندد، نبود رد می‌شود.
     */
    if (verb === 'click' && CLOSERS.test(text.trim())) {
...
/**
 * این سناریو ضبطِ خام است یا مسیرِ ورودِ ساخته‌شده؟
 *
 * ── چرا این تشخیص لازم شد ──
 *
 * کاربر گشت رفت، و بعد در صفحهٔ نقشه **همان پیش‌نویسِ گشت** را به‌عنوان
 * «مسیرِ ورود» انتخاب کرد — که کاملاً طبیعی است، چون تنها گزینهٔ کشویی بود.
 * سه خزش پشتِ سرِ هم با صفر قدم مرد:
 *
 *     locator.click: Timeout — getByRole('button', { name: 'نشان نده' })
 *
 * ضبطِ گشت کلیک‌های بی‌شرط دارد، از جمله بستنِ مودالی که فقط بارِ اول
 * می‌آید. خزش ده‌ها بار به خانه برمی‌گردد و بارِ دوم آن دکمه نیست.
 *
 * نشانه‌اش ساده است: مسیرِ ورودِ درست، کنش‌هایش زیر `when` است. ضبطِ خام،
 * زنجیره‌ای از کلیکِ بی‌شرط.
 */
export function looksRecorded(steps = []) {
  const bare = steps.filter((step) => {
    const verb = stepVerb(step);
    return ['click', 'fill', 'check', 'press', 'dblclick', 'hover'].includes(verb);
  });
