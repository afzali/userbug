...
export const SNAPSHOT_FN = () => {
  ...
  for (const el of document.querySelectorAll(INTERACTIVE)) {
    ...
    items.push({
       ...
       * نشانِ ضبط‌کننده.
       *
       * ── چرا این دو خط اینجاست و نه در ماژول گشت ──
       *
       * ضبط‌کننده باید بداند کاربر روی **کدام** عنصرِ همین فهرست کلیک کرد، تا
       * `descriptorFor` بتواند همان توصیفِ پایدار را بسازد که `do:` می‌سازد.
       * اگر گشت فهرستِ خودش را می‌ساخت، دو تعریفِ «عنصرِ قابل اشاره» داشتیم و
       * دیر یا زود واگرا می‌شدند — یعنی سناریوی ضبط‌شده به عنصری اشاره می‌کرد
       * که کش هرگز پیدایش نمی‌کرد.
       *
       * در اجرای عادی همیشه `undefined` است و هزینه‌اش یک `getAttribute` است.
       */
      rec: el.getAttribute('data-ub-rec') || undefined,
      role: role || undefined,
      name,
      label,
      placeholder,
