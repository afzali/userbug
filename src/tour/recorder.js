...
export function recorderScript() {
   ...
   */
  /*
   * ── چرا از sessionStorage خوانده می‌شود ──
   *
   * این اسکریپت با **هر ناوبری** از نو اجرا می‌شود، در یک محیطِ جاوااسکریپتِ
   * تازه. پس نوشتنِ مستقیمِ مقدارِ true یعنی هر بار که کاربر صفحه را عوض
   * کند، ضبط خودش دوباره روشن می‌شود — و دکمهٔ «توقف ضبط» عملاً فقط تا
   * کلیکِ بعدی کار می‌کرد.
   *
   * نود هم نمی‌تواند این را جبران کند: تا وقتی رخدادِ ناوبری به نود برسد و
   * evaluate برگردد، چند کلیک ضبط شده‌اند.
   *
   * sessionStorage در همان تبِ مرورگر و بینِ ناوبری‌ها زنده می‌ماند، پس
   * تصمیمِ کاربر همان‌جا می‌ماند که لازم است — پیش از اجرای هر کدِ صفحه.
   *
   * (اینجا بک‌تیک نمی‌گذاریم: این متن داخل یک template literal است و
   * بک‌تیک رشته را می‌بندد. یک بار همین‌جا شکست.)
   */
  try {
    const saved = sessionStorage.getItem('__ubRecording');
    window.__ubRecording = saved === null ? true : saved === '1';
  } catch {
    // حالتِ خصوصی یا مبدأی که انبار ندارد: پیش‌فرضِ روشن، چون خاموشِ خاموش
    // بدترین حالت است — گشت کار می‌کند و هیچ قدمی ضبط نمی‌شود.
