<script>
  ...
  /**
   * «چند endpoint اصلاً صدا زده شده».
   *
   * ── چرا این خط لازم بود ──
   *
   * markup سال‌ها `touched` را می‌خواند بی آنکه جایی تعریف شده باشد، و
   * هیچ‌کس ندید: این بلوک فقط وقتی رندر می‌شود که سورس **یک بار** خوانده
   * شده باشد، و پروژه‌های موجود همه از قبل خوانده بودند. روی یک پروژهٔ تازه،
   * درست بعدِ «خواندنِ دوبارهٔ سورس»، کلِ صفحهٔ کشف ۵۰۰ می‌داد.
   *
   * `svelte-check` هم نگرفت؛ چیزی که گرفتش یک بار استفادهٔ واقعی بود.
   */
  let touched = $derived(
    Math.max(0, (data.found.endpoints?.total || 0) - (data.found.endpoints?.untouched?.length || 0))
  );
   ...
   *
   * ── چرا بقیهٔ متغیرهای نقشه از اینجا رفتند ──
   *
   * `map`، `states`، `families`، `totals` و `kindsOf` همه برای «گزارشِ
   * نقشه» بودند که به `MapReport` رفت و در جلسهٔ خزش نشست. آن عددها
   * دربارهٔ **یک خزش** اند، نه دربارهٔ چیزی که برای همیشه می‌دانیم.
   */
