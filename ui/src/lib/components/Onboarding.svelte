<script>
  ...
  const SLIDES = $derived([
    ...
    },
    {
      ...
      what: [
        ...
        'و در <strong>دانسته‌ها</strong> به پرسش‌های بی‌جواب جواب بدهید؛ همان‌ها هستند که سناریوی بعدی را دقیق‌تر می‌کنند.',
      ],
      /**
       * ── چرا این قدم دو مقصد داشت و حالا یکی ──
       *
       * «این اپ چه دارد» را حالا درختِ «اپِ من» جواب می‌دهد و «از کجا
       * می‌دانیمش» را «دانسته‌ها». هر دو از صفحهٔ کشف بیرون آمدند، چون
       * آنجا کنارِ ابزارِ گشتن نشسته بودند و فقط فاصله بودند.
       */
      href: base,
      cta: 'رفتن به «اپِ من»',
    },
    ...
    },
    {
      ...
      what: [
        ...
        'در <strong>اپِ من</strong> می‌بینید هر بخشِ اپ چند سناریو دارد، چند بار آزموده شده، و چه ایرادی داشته.',
      ],
      href: base,
      cta: 'بررسی',
    },
    {
      ...
      what: [
        '<strong>نوارِ بالای هر صفحه</strong> بنویسید «برو بگرد ببین باگی هست» یا «تست‌ها را بگیر». پیش از اجرا نشان می‌دهد چه می‌خواهد بزند.',
   ...
   * حالت `/files` یا `/missions`. تطبیقِ رشته‌ای روی آدرس، همیشه همین‌طور
   * بی‌صدا می‌شکند.
   *
