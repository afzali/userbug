<script>
   ...
   */
  /**
   * چهار ردیف — و هر چهارتا یک پرسشِ کاربرند، نه یک قابلیتِ ابزار.
   *
   * ── چرا «کشف» و «بررسی» از منو رفتند ──
   *
   * هیچ‌کدام **مقصد** نیستند؛ کارند. ردیفِ منو یعنی کاربر باید اول به یک
   * صفحه برود و بعد تازه کار را شروع کند. حالا دو دکمه‌اند روی همان صفحه‌ای
   * که رویش کار می‌شود، و دامنه‌شان از همان درخت می‌آید.
   *
   * تاریخچه‌شان هم مقصد نیست — کاربر گفت «اینکه چگونه کشف کردیم و چند بار،
   * به ذات مهم نیست». هر دو در «اجراها» می‌نشینند، در دو دستهٔ روشن.
   *
   * ── چرا دو ردیفِ آخر «پیشرفته» هستند ──
   *
   * سناریو خروجیِ کشف است و کاربر لزوماً درگیرش نمی‌شود؛ اجراها هم
   * تاریخچه است. هر دو لازم‌اند و هیچ‌کدام کارِ روزانه نیستند.
   */
  let nav = $derived(
    inProject
      ? [
            ...
            items: [
              {
                ...
                hint: counts
                  ? counts.caps
                    ? `${fa(counts.caps)} بخش${counts.blind ? ` · ${fa(counts.blind)} بی‌سناریو` : ''}`
                    : 'هنوز چیزی کشف نشده'
                  : 'بخش‌ها و قابلیت‌ها',
                title: 'نقشهٔ اپ — هر بخش و قابلیت، با اینکه بررسی شده یا نه. کشف و بررسی هم از همین‌جا شروع می‌شوند.',
              },
              {
                href: `${base}/triage`,
                label: 'یافته‌ها',
                ...
                title: 'خروجیِ بررسی‌ها: چه ایرادی، کجا، چند بار — و نسبت به بارِ قبل چه تازه است.',
              },
            ],
          },
          {
            ...
            items: [
              {
                ...
                hint: counts
                  ? counts.missions
                    ? `${fa(counts.green)} سالم از ${fa(counts.missions)}`
                    : 'هنوز سناریویی نیست'
                  : 'بلوپرینتِ آزمون‌ها',
                title: 'خروجیِ کشف‌ها: هر سناریو یک فایل، با وضعیتش و اینکه چیزی را می‌سنجد یا فقط می‌گوید «چیزی نشکست». لازم نیست درگیرشان شوید.',
              },
              {
                href: `${base}/runs`,
                label: 'اجراها',
                icon: '◎',
                hint: 'کشفی و بررسی',
                title: 'هر بار که چیزی اجرا شد — کشف یا بررسی. داخلِ هرکدام می‌شود رفت و دید چه گرفت.',
              },
            ],
          },
        ]
            ...
            items: [
              {
                ...
                title: 'فهرست اپ‌ها، با آخرین بررسیِ هرکدام.',
              },
              {
                href: '/projects/new',
                label: 'اپِ تازه',
