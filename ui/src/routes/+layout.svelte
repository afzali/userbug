<script>
  ...
  /**
   * عددِ زندهٔ هر ردیف — جای نوارِ پیشرفتِ صفحهٔ خانه.
   *
   * ── چرا از آن صفحه به اینجا آمد ──
   *
   * آن نوار پنج قدم را با عدد نشان می‌داد و درست بود، ولی فقط روی **یک**
   * صفحه دیده می‌شد: به محضِ رفتن به «کشف» یا «یافته‌ها»، «کجاییم» گم
   * می‌شد. کاربر گفت «همهٔ اینها هم معلوم باشد که کجاییم و چه باید بکنیم».
   * منو تنها چیزی است که همه‌جا هست.
   *
   * نبودِ `counts` (صفحه‌های بیرونِ پروژه) با صفر بودنش فرق دارد: اولی یعنی
   * نمی‌دانیم، دومی یعنی هیچ. پس زیرنویسِ ثابت سرِ جایش می‌ماند و فقط وقتی
   * عدد داریم جایش را می‌دهد.
   */
  let counts = $derived(page.data?.counts || null);
      ...
      ? [
            ...
            items: [
              {
                 ...
                 * ── چرا «اپِ من» خانه شد ──
                 *
                 * خانه «اجرا» بود: یک فرمِ بیست‌کنترلی از سناریو و دستگاه و
                 * پرسونا و مدل. آن فرم پیکربندیِ **ابزار** است، و اولین
                 * چیزی که آدم هر روز می‌بیند نباید تنظیماتِ ماشین باشد.
                 *
                 * پرسشی که کاربر صبح با آن می‌آید دربارهٔ اپِ خودش است —
                 * «چه دارد، کدامش را فراموش کرده‌ام، کجا شکست» — و تا امروز
                 * هیچ صفحه‌ای این را نمی‌پرسید.
                 */
                href: base,
                label: 'اپِ من',
                icon: '◳',
                ...
                hint: counts
                  ? counts.caps
                    ? `${fa(counts.caps)} قابلیت${counts.blind ? ` · ${fa(counts.blind)} بی‌سناریو` : ''}`
                    : 'هنوز چیزی کشف نشده'
                  : 'بخش‌ها و قابلیت‌ها',
                title: 'درختِ بخش‌ها و قابلیت‌های اپ — هر کدام با شمارِ سناریو، اجرا و ایراد. از همین‌جا می‌شود گفت «برو این بخش را بگرد».',
              },
              {
                /**
                 * ── چرا «کشف» به ردیف‌های اصلی برگشت ──
                 *
                 * یک بار از منو بیرونش بردم با این استدلال که «کار است نه
                 * مقصد». آن استدلال غلط بود: کشف **تاریخچه** دارد — هر
                 * گشت و هر خزش یک جلسه است که بعداً می‌شود بازش کرد و دید
                 * چه پیدا کرد.
                 *
                 * و حالا هم‌شکلِ «بررسی» است: فهرستِ جلسه‌ها، دکمهٔ «تازه»،
                 * و رفتن داخلِ یکی. دو صفحهٔ هم‌شکل یعنی یاد گرفتنِ یکی،
                 * یاد گرفتنِ آن یکی.
                 */
                href: `${base}/discover`,
                label: 'کشف',
                icon: '◱',
                hint: counts
                  ? counts.pages || counts.states
                    ? `${fa(counts.pages)} صفحه · ${fa(counts.states)} حالت`
                    : 'هنوز نگشته‌ایم'
                  : 'بگرد، ببین چه دارد',
                title: 'هر بارِ کشف یک ردیف — گشتِ زنده، خزش، یا خواندنِ سورس. نتیجه‌شان در «اپِ من» جمع می‌شود.',
              },
              {
                /**
                 * ── چرا «اجرا» و «دورها» یکی شدند ──
                 *
                 * دو ردیف بودند و استدلالش این بود که «دو پرسشِ متفاوت‌اند».
                 * نبودند: هر دو دربارهٔ همان یک کار بودند و کاربر نمی‌دانست
                 * کدام را بزند. و نامش «بررسی» است نه «اجرا»: «اجرا» کارِ
                 * ابزار را می‌گوید، «بررسی» کاری را که آدم می‌کند.
                 *
                 * ── و چرا کنارِ «کشف» نشسته ──
                 *
                 * هر دو صندوق‌اند: فهرستِ جلسه‌ها، دکمهٔ «تازه»، و رفتن
                 * داخلِ یکی. کشف می‌فهمد اپ چه دارد، بررسی می‌آزمایدش.
                 */
                href: `${base}/rounds`,
                label: 'بررسی',
                icon: '▶',
                hint: 'بررسی کن، و تفاوتش را ببین',
                title: 'اپ را بیازمایید — همه یا بخشی از آن — و ببینید بارِ گذشته چه دید و این بار چه یافتهٔ تازه‌ای آمد و چه رفع شد.',
              },
              {
                href: `${base}/triage`,
                label: 'یافته‌ها',
                icon: '◇',
                hint: counts ? (counts.open ? `${fa(counts.open)} بازِ بی‌قضاوت` : 'همه قضاوت شده') : 'ایرادها و وضعیتشان',
          ...
          },
          {
            ...
            items: [
              {
                /**
                 * ── چرا «دانسته‌ها» صفحهٔ خودش را گرفت ──
                 *
                 * سیزده بخش از صفحهٔ کشف اینجا آمدند — واژه‌نامه، خطرها،
                 * مستندات، تاریخچهٔ شناخت، endpointها. هیچ‌کدام ربطی به
                 * «الان دارم می‌گردم» نداشتند و فقط فاصله بودند تا چیزی که
                 * دنبالش هستی.
                 *
                 * یک جنس‌اند و یک پرسش: چه می‌دانیم، و از کجا آمده.
                 */
                href: `${base}/knowledge`,
                label: 'دانسته‌ها',
                icon: '◈',
                hint: 'چه می‌دانیم، از کجا',
                title: 'توضیحِ پروژه، استک و ورود، واژه‌نامه، خطرها، مستنداتِ بیرونی، پرسش‌ها، تاریخچهٔ شناخت، و endpointها و ناورداهای سورس.',
              },
              {
                /**
                 * ── چرا به «مرجع» رفت ──
                 *
                 * وقتی تنها فهرستِ «کاربر چه سفرهایی می‌رود» بود، کارِ روزانه
                 * حساب می‌شد. حالا آن پرسش را درخت جواب می‌دهد و اینجا چیزِ
                 * باریک‌تری مانده: کدام **فایلِ آزمون** سبز است و چند انتظار
                 * دارد. سراغش می‌روی با یک سؤالِ مشخص، نه هر روز.
                 *
                 * ── و چرا دوباره «سناریوها» شد ──
                 *
                 * «مأموریت» انتخابِ درستی بود وقتی این صفحه تنها جایی بود که
                 * می‌گفت «کاربر چه سفرهایی می‌رود». حالا آن پرسش را درختِ
                 * «اپِ من» جواب می‌دهد، و اینجا چیزِ باریک‌تری مانده: کدام
                 * **فایلِ آزمون** سبز است و چند انتظار دارد.
                 *
                 * و یک برخوردِ واقعی هم حل می‌شود: روی دیسک
                 * `knowledge/<هدف>/missions/*.json` چیزِ کاملاً دیگری است —
                 * هدفِ یک کاوش. یک نام برای دو چیز، همان ابهامی است که این
                 * بازچینش برای حذفش شروع شد.
                 */
                href: `${base}/missions`,
                label: 'سناریوها',
                icon: '⌘',
                hint: counts
                  ? counts.missions
                    ? `${fa(counts.green)} سالم از ${fa(counts.missions)}${counts.red ? ` · ${fa(counts.red)} قرمز` : ''}`
                    : 'هنوز سفری نیست'
                  : 'سفرها و سلامتشان',
