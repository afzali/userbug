<script>
  ...
  // svelte-ignore state_referenced_locally
  let target = $state(data.projects[0]?.key || '');
  let scenario = $state('');
  let device = $state('');
  let persona = $state('');
  let depth = $state('');
  ...
  let project = $derived(data.projects.find((item) => item.key === target));
...
<PageHeader eyebrow="فاز ۳ · کنترل‌پنل محلی" title="اجرای سناریو مثل یک کاربر واقعی" description="هدف و سناریو را انتخاب کنید؛ قدم، عکس، خطای مرورگر و لاگ سرور در همان لحظه اینجا می‌آیند.">
  {#snippet actions()}
    <Button href="/files" variant="outline">ویرایش سناریو</Button>
    <Button href="/compare" variant="outline">مقایسهٔ اجراها</Button>
  {/snippet}
</PageHeader>
    ...
    <Card.Content>
      <form class="space-y-4" onsubmit={start}>
        <label class="block space-y-1.5 text-sm font-medium">
          <span>پروژه</span>
          <select class="app-select" bind:value={target} onchange={changeTarget} disabled={busy}>
            {#each data.projects as item}<option value={item.key}>{item.name} · {item.environment}</option>{/each}
          </select>
        </label>
        <label class="block space-y-1.5 text-sm font-medium">
          <span>سناریو</span>
