<script>
  ...
  const WAYS = [
    ...
    },
    {
       ...
       *
       * یک پروژهٔ تازه ساختم و همین گزینه را زدم. خزنده از صفحهٔ ورود رد
       * نشد: دو حالت، صفِ صفر، پنج قدم. اپ ورود داشت و خزنده حسابی
       * نداشت که با آن وارد شود.
       *
       * مخزن این تله را از قبل می‌شناسد — در راهنمای `map` نوشته شده
       * «نقشه بعد از گشت می‌آید، نه پیش از آن». ولی این صفحه هر سه راه را
       * هم‌وزن نشان می‌داد و هیچ‌جا نگفت.
       *
       * دانشی که فقط در مستندات باشد، در لحظهٔ تصمیم وجود ندارد.
       */
      warn:
        'اگر اپ ورود دارد، خزنده معمولاً روی همان صفحهٔ ورود می‌ماند و چیزی ' +
        'پیدا نمی‌کند — چون هنوز حسابی ندارد که با آن وارد شود. در آن حالت ' +
        'اول «خودم نشانت می‌دهم» را بروید؛ یک بارِ ورودِ دستی کافی است.',
    },
    {
      key: 'scoped',
        ...
        >
          ...
          <span class="min-w-0">
            ...
            {#if way.warn && how === way.key}
              <!--
                هشدار فقط وقتی همین گزینه انتخاب شده.

                همیشه نشان دادنش یعنی سه کادرِ زرد کنارِ هم که هیچ‌کدام
                خوانده نمی‌شوند. لحظهٔ درستش همان است که آدم انتخابش کرده و
                هنوز «شروع» را نزده.
              -->
